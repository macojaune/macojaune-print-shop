...
import { createStore, missions, project, STORAGE_KEY, statusFor, completedMainCount, deriveWorldState } from './model.js';
...
test('new state opens inscriptions and the bonus, with sequential main dependencies', () => {
  const state = createStore({ storage: null }).getState();
  assert.equal(state.schema, 1);
  assert.deepEqual(state.counts, { inscrits: 7, retours: 0, boutiques: 0, bonus: 0 });
...
test('blocked contributions cannot alter counts, consume events, persist, or notify', () => {
  const storage = memoryStorage();
  const store = createStore({ storage });
  const before = store.getState();
  let notifications = 0;
  store.subscribe(() => { notifications += 1; });
  assert.equal(store.contribute('retours', 'early-return').reason, 'blocked');
  assert.deepEqual(store.getState(), before);
  assert.equal(storage.writes, 0);
  assert.equal(notifications, 0);
  contributeTimes(store, 'inscrits', 3);
  assert.equal(statusFor('retours', store.getState()), 'open');
  assert.equal(statusFor('boutiques', store.getState()), 'blocked');
...
test('a contribution event is idempotent globally, including after reload', () => {
  ...
  assert.equal(store.getState().counts.inscrits, 8);
...
test('only all three main missions repair the vehicle', () => {
  ...
  assert.equal(store.getState().repaired, false);
  contributeTimes(store, 'retours', 3);
...
test('presets deliberately replace progress, dedupe history and the repair latch', () => {
  ...
  assert.deepEqual(store.getState().counts, { inscrits: 10, retours: 1, boutiques: 0, bonus: 0 });
  assert.deepEqual(store.getState().completed, ['inscrits']);
  assert.deepEqual(store.getState().seenEvents, []);
  store.preset('repaired');
  assert.deepEqual(store.getState().counts, { inscrits: 10, retours: 3, boutiques: 5, bonus: 0 });
  assert.equal(store.getState().repaired, true);
  store.preset('bonus');
  assert.deepEqual(store.getState().counts, { inscrits: 7, retours: 0, boutiques: 0, bonus: 3 });
  ...
  assert.deepEqual(store.getState(), before);
  store.reset();
  assert.deepEqual(store.getState().counts, { inscrits: 7, retours: 0, boutiques: 0, bonus: 0 });
...
test('state snapshots, mutation results and subscriber snapshots cannot change store state', () => {
  ...
  assert.equal(store.getState().counts.inscrits, 8);
  assert.deepEqual(store.getState().completed, []);
  assert.equal(calls[0].counts.inscrits, 8);
  unsubscribe();
  store.contribute('inscrits', 'second');
  assert.equal(calls[1].counts.inscrits, 9);
});
...
test('invalid storage data recovers without accepting nonnumeric counts or prototype ids', () => {
  for (const value of ['{broken', 'null', '[]', '{"schema":2,"counts":{}}']) {
    ...
    assert.equal(store.getState().counts.inscrits, 7);
    assert.deepEqual(store.getState().completed, []);
  }
  ...
  const store = createStore({ storage: memoryStorage(value) });
  assert.deepEqual(store.getState().counts, { inscrits: 7, retours: 0, boutiques: 0, bonus: 2 });
  assert.deepEqual(store.getState().completed, []);
...
test('storage denial and quota failure leave an operational session store', () => {
  ...
  assert.equal(quota.getState().counts.inscrits, 8);
  assert.doesNotThrow(() => createStore());
});

test('world projection preserves selection without exposing store arrays', () => {
  const store = createStore({ storage: null });
  store.preset('repaired');
