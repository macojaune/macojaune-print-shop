...
test('v6 persistence never reads or replaces the v4 demonstration', () => {
  const values = new Map([['macomisyon-warehouse-v4', '{"schema":1,"counts":{"inscrits":10}}']]);
  const originalV4 = values.get('macomisyon-warehouse-v4');
  const store = createStore({ storage: {
    getItem(key) { return values.get(key) ?? null; },
    setItem(key, value) { values.set(key, value); },
  } });
  assert.equal(STORAGE_KEY, 'macomisyon-warehouse-v6');
  assert.equal(store.getState().counts.inscrits, 2);
  store.contribute('inscrits', 'v5-first');
  assert.equal(values.get('macomisyon-warehouse-v4'), originalV4);
