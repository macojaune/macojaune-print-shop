...
export function createWorld({canvas,container,labels,onSelect}) {
  ...
  function repairTarget() {
    ...
    return {tilt:upright?0:-1.48,hood:upright?0:-.72,mast:gathered?0:-.12,gather:gathered?1:0,lift:state.repaired?.1:0,light:state.repaired?1:state.completed.includes('boutiques')?.25:0,door:state.repaired?1:state.completed.includes('boutiques')?.22:0,dispatch:state.repaired?1:0};
  }
  ...
  function applySignals(){
    ...
    alarmMix=clamp(-(visualPose?.tilt??-1.48)/1.48,0,1);
  ...
  function setState(next,{animate=false,arrival=false}={}) {
    ...
    if(arrival&&mechanicalChanged){
      from=repairTarget();
      if(state.repaired)Object.assign(from,{dispatch:0,door:0,light:0,lift:0});
      ...
      else if(state.completed.includes('inscrits'))Object.assign(from,{tilt:-1.48,hood:-.72});
    }
