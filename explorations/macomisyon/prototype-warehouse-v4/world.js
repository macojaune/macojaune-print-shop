...
export function createWorld({canvas,container,labels,onSelect}) {
  ...
  const shouldAnimate=()=>active()&&!reduced.matches&&(animation!==null||cameraFlight!==null||state.repaired);
  ...
  function prepareScene() {
    ...
    parcel(scenery,{position:[6.17,.07,.51],size:[.72,.77,.75],tone:1});
    parcel(scenery,{position:[6.12,.85,.48],size:[.62,.45,.63],tone:0,rotation:.12});
  ...
  function applyRepairPose(pose) {
    visualPose={...pose};dispatchProgress=pose.dispatch;
    ...
    const x=THREE.MathUtils.lerp(THREE.MathUtils.lerp(baseX,3.85,travelA),4.7,travelB);
    const z=THREE.MathUtils.lerp(THREE.MathUtils.lerp(.03,1.75,travelA),-1.63,travelB);
    const yaw=THREE.MathUtils.lerp(THREE.MathUtils.lerp(-.14,Math.PI/2,turnA),Math.PI,turnB);
    ...
    const lift=d>0&&d<1?.68*phase(d,0,.18)*(1-.853*phase(d,.76,.9)):pose.lift;
    ...
    cargoRoot.position.set(0,-.1*exit,-1.93*exit);
    cargoRoot.visible=exit<.985;
    const door=d>0&&d<1?phase(d,.12,.47):pose.door;
  ...
  function setState(next,{animate=false}={}) {
    ...
    if(animate&&from&&!reduced.matches&&active()){
      ...
      if(Object.keys(to).some(key=>Math.abs(to[key]-from[key])>.001)){
        animation={from,to,start:performance.now(),duration:state.repaired?5500:1150};
  ...
  function render(time) {
    ...
    if(cameraFlight){
      const progress=clamp((time-cameraFlight.start)/650,0,1),ease=1-(1-progress)**3;
  ...
  function focusNode(id){
    ...
    return new Promise(resolve=>{cameraFlight={id,from:target.clone(),to,fromZoom:zoom,toZoom,start:performance.now(),resolve};invalidate();});
  }
  ...
  try{
    ...
    addListener(document,'visibilitychange',visibilityChanged);addListener(reduced,'change',()=>{if(reduced.matches){if(animation){animation=null;applyRepairPose(repairTarget());}cancelFlight({finish:true});}invalidate();});
  ...
  return {setState,focusNode,resetView,zoomBy,getDebug,projectPoint,dispose};
}
