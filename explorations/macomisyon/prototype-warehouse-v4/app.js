...
function updateDock(state){
 ...
 if(selected){
  ...
  $('depotText').textContent=state.repaired?'La cargaison peut enfin partir.':n?'Chaque réussite remet une pièce en place.':'Colis éparpillés. Trois missions pour repartir.';
...
updateDock(previous);
try{world=createWorld({canvas:$('scene'),container:$('world'),labels:$('labels'),onSelect:id=>{++locating;openPanel(id==='project'?'project':'mission',id);}});world.setState(worldState(previous),{animate:false});$('loading').hidden=true;}catch(error){$('loading').hidden=true;$('fallback').hidden=false;console.error(error);}
