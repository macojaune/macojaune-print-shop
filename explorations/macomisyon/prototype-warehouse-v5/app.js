...
const motionKey='macomisyon-warehouse-v5-motion';
const motionPreference=window.matchMedia('(prefers-reduced-motion: reduce)');
let motionPaused=false;
try{motionPaused=localStorage.getItem(motionKey)==='paused';}catch{}
function motionControlHTML(){return `${icon(motionPreference.matches?'pause':motionPaused?'play':'pause')}<span>${motionPreference.matches?'Mode calme':motionPaused?'Reprendre':'Pause'}</span>`;}
function updateMotionControl(){
 const button=$('motionButton');button.innerHTML=motionControlHTML();button.disabled=motionPreference.matches;
 button.setAttribute('aria-pressed',String(motionPaused||motionPreference.matches));button.setAttribute('aria-label',motionPreference.matches?'Mode calme activé sur cet appareil':motionPaused?'Reprendre les animations du dépôt':'Mettre les animations du dépôt en pause');button.title=button.getAttribute('aria-label');
 world?.setMotionPaused(motionPaused);
}
function toggleMotion(){
 if(motionPreference.matches)return;motionPaused=!motionPaused;
...
$('motionButton').onclick=toggleMotion;
 motionPreference.addEventListener('change',()=>{updateMotionControl();if(panel?.type==='demo')renderPanel();});
...
updateDock(previous);
try{world=createWorld({canvas:$('scene'),container:$('world'),labels:$('labels'),onSelect:id=>{++locating;openPanel(id==='project'?'project':'mission',id);}});world.setMotionPaused(motionPaused);world.setState(worldState(previous),{animate:false});$('loading').hidden=true;}catch(error){$('loading').hidden=true;$('fallback').hidden=false;console.error(error);}
updateMotionControl();
