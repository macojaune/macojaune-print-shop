  ...
  .page-payer.px-4
    ...
    .info.mt-8(v-show="state.show")
      p.mb-4.text-2xl.text-white.text-center Régler
        span.text-4xl.font-bold.text-orange-400.mx-2(@click="state.show=false") {{ state.amount}}€
        |
        span.text-white.mr-2 ?
        |
        span.text-xl Simple comme bonjour.
    .payment(v-show="state.show")
      p.text-center.text-white Insère tes informations en toute tranquilité.
...
const state = reactive({
  ...
  loading: false,
  intent: {},
  amount: route.params.amount as string,
