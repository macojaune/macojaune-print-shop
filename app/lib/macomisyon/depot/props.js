...
// The Nuxt route can be entered and left repeatedly in a single browser session.
// Release cached GPU resources together with the scene instead of retaining them
// behind this module after the component is gone.
export function disposeAssetCache() {
  for (const geometry of geometries.values()) geometry.dispose();
  for (const mat of materials.values()) mat.dispose();
  for (const mat of labelMaterials.values()) { mat.map?.dispose(); mat.dispose(); }
  geometries.clear(); materials.clear(); labelMaterials.clear();
}
export const colors = {
...
export function forklift(parent,{position=[0,0,0],rotation=-.18,loaded=true}={}) {
  ...
  // A broad side plate stays readable on the exposed flank when the truck is toppled.
  const sideQ=sign(body,'Q',[.71,.56,.027],[ -.728,.91,-.62],{background:'#ffd03a',width:128,height:128});sideQ.rotation.y=-Math.PI/2;
  const roofQ=sign(body,'Q',[.71,.68,.021],[0,2.599,.03],{background:'#ffd03a',width:128,height:128});roofQ.rotation.x=-Math.PI/2;
  ...
  const unplugged=tube(g,[[.48,1.32,-.66],[.69,1.14,-.79],[.73,.68,-.88],[.67,.22,-1.23],[.51,.13,-1.45]],.039,colors.ink);
  ...
  const beacon=cylinder(g,.16,.25,[.46,2.69,-.42],lampMat.clone());
  cylinder(g,.18,.06,[.46,2.56,-.42],colors.ink);
  const headlights=[];
