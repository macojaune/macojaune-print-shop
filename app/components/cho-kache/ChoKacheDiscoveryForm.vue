    ...
    <div v-else class="grid gap-10 lg:grid-cols-12 lg:gap-14">
      <div class="lg:col-span-4">
        ...
        <p class="mb-0 mt-6 max-w-[30rem] text-base leading-7 text-stone-300">
          Entre le code au dos et laisse-moi ton @ ou ton email.
        </p>
        <button
          type="button"
      ...
      <form class="min-w-0 space-y-8 lg:col-span-8" novalidate @submit.prevent="submitDiscovery">
        <div>
          ...
          <template v-else>
            ...
            <p id="discovery-code-hint" class="mb-0 mt-2 text-sm leading-6 text-stone-400">
              Il est imprimé derrière le tirage.
            </p>
        ...
        <div class="min-w-0">
          ...
          <input
            ...
            :aria-describedby="fieldErrors.contact ? 'discovery-contact-error' : undefined"
          >
          <p v-if="fieldErrors.contact" id="discovery-contact-error" class="mb-0 mt-2 text-sm text-amber-200" role="alert">
            {{ fieldErrors.contact }}
          </p>
        </div>
          ...
          <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p class="mb-0 max-w-[34rem] text-sm leading-6 text-stone-400">
                Elle ne part que si tu appuies sur le bouton. Tu peux la retirer avant l'envoi.
              </p>
            </div>
            <button
        ...
        <div class="border-y border-white/12 py-6">
          <div class="mb-5">
            <p class="mb-1 font-display text-3xl uppercase leading-none text-white sm:text-4xl">
              Poste ta trouvaille
            </p>
            <p class="mb-0 max-w-[35rem] text-sm leading-6 text-stone-300">
              Choisis tes photos ou vidéos. Tu pourras les poster en mentionnant @macojaune ou me les envoyer avec le formulaire.
            </p>
          </div>

          <label for="discovery-media" class="mb-2 block text-sm font-semibold text-white">
            Tes photos ou vidéos
          </label>
          <input
            id="discovery-media"
            ref="mediaInput"
          ...
          <p class="mb-0 mt-2 text-sm leading-6 text-stone-400">
            Choisis-en autant que tu veux, jusqu'à 50 Mo par envoi. Tu peux les partager ou me les envoyer avec le formulaire.
          </p>
          ...
          <button
            type="button"
            :disabled="form.media.length === 0 || Boolean(fieldErrors.media)"
            class="mt-4 inline-flex min-h-11 w-full items-center justify-center border border-amber-300/55 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-amber-200 transition hover:bg-amber-300 hover:text-black focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-300 disabled:pointer-events-none disabled:border-white/15 disabled:text-white/35 sm:w-auto"
            @click="shareDiscovery"
          >
            Partager avec @macojaune
          </button>
          <p v-if="fieldErrors.media" class="mb-0 mt-2 text-sm text-amber-200" role="alert">
            {{ fieldErrors.media }}
          </p>
          <p v-if="shareStatus" class="mb-0 mt-3 text-sm text-amber-200" role="status">
            {{ shareStatus }}
          </p>
          <p v-if="shareError" class="mb-0 mt-3 text-sm text-amber-200" role="alert">
            {{ shareError }}
          </p>
        </div>
        ...
        <label class="flex cursor-pointer items-start gap-3 border border-white/15 bg-black/45 p-4 text-sm leading-6 text-stone-300">
          ...
          <span>Tu peux aussi repartager ces photos ou vidéos sur les comptes de @macojaune.</span>
        </label>
...
const validateForm = () => {
  ...
  if (!form.contact.trim()) {
    fieldErrors.contact = 'Laisse-moi ton @ ou ton email pour que je puisse te répondre.'
    isValid = false
  }

  const totalSize = form.media.reduce((total, file) => total + file.size, 0)
  const invalidFile = form.media.find(file => !allowedMediaTypes.has(file.type))

  if (invalidFile) {
    fieldErrors.media = "Le format d'un fichier n'est pas accepté."
    isValid = false
...
const shareDiscovery = async () => {
  ...
  const text = "J'ai trouvé une photo Cho Kaché de @macojaune."
  const url = `${window.location.origin}/cho-kache`
  ...
  if (form.media.length && navigator.canShare?.({ files: form.media })) {
    shareData.files = form.media
  }

  try {
    if (navigator.share) {
      await navigator.share(shareData)
      shareStatus.value = shareData.files?.length
        ? 'Partage ouvert avec tes fichiers et la mention @macojaune.'
        : 'Partage ouvert avec la mention @macojaune.'
      return
    }

    await navigator.clipboard.writeText(`${text} ${url}`)
    shareStatus.value = 'Texte copié. Tu peux le coller dans ton post.'
  } catch (error) {
    if (error instanceof DOMException && error.name === 'AbortError') {
      return
    }

    shareError.value = "Le partage ne s'est pas ouvert. Tu peux poster directement en mentionnant @macojaune."
  }
}
...
const requestLocation = () => {
  ...
  if (!navigator.geolocation) {
    locationError.value = "Ton navigateur ne permet pas d'ajouter la position."
    return
  }
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    () => {
      locationError.value = "Je n'ai pas pu récupérer ta position. Tu peux réessayer."
      isLocating.value = false
    },
    {
