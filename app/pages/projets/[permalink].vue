...
to="/projets"
                    ...
                    <div class="mt-3 flex items-start gap-3 lg:gap-4">
                        ...
                        <h1
                            class="max-w-[11ch] font-display text-5xl uppercase leading-[0.84] text-amber-400 lg:text-[6.2rem]">
                            {{ data?.title ?? "" }}
                        </h1>
                    </div>
                    <div
...
v-if="projectImages.length"
                            ...
                            <div class="project-image-stage relative aspect-[4/5] w-full overflow-hidden">
                                <ProjectImage
v-for="(image, index) in projectImages" :key="`${image}-${index}`"
                                    :src="image" :alt="data?.title ?? ''" :loading="index === 0 ? 'eager' : 'lazy'"
                                    :fetchpriority="index === 0 ? 'high' : 'auto'" :class="[
                            ...
                            <figcaption
                                class="absolute left-3 top-3 border border-black/20 bg-black/60 px-3 py-2 text-[10px] uppercase tracking-[0.28em] text-amber-100 backdrop-blur-sm">
...
v-for="(image, index) in projectImages" :key="`dot-${image}-${index}`"
                                ...
                                <span
                                    class="project-image-counter text-[8px] uppercase tracking-[0.24em] text-amber-50/90">
                                    {{ String(activeProjectImageIndex + 1).padStart(2, '0') }}/{{
                    ...
                    <section v-if="data?.pinterestUrl" class="mt-10 space-y-4 lg:mt-12">
                        ...
                        <div class="overflow-hidden bg-stone-950/55 p-3 lg:p-4">
                            <a
data-pin-do="embedBoard" data-pin-scale-height="520" data-pin-scale-width="100%"
                                :href="data?.pinterestUrl" />
                        </div>
                    </section>
                </article>

                <aside ref="asideRef" class="lg:relative lg:col-span-4 lg:self-start" :style="desktopAsideStyle">
                    <div
ref="formPanelRef"
                        ...
                        <p class="max-w-[28ch] text-base leading-7 text-stone-200">
                            Très simple, accorde-moi 24 secondes et demi pour remplir ce court formulaire et c&apos;est
                            parti.
                        </p>
                        ...
                        <iframe
                            ...
                            title="Participer à un Projet Photo avec Macojaune" class="min-h-[638px] bg-transparent" />
                    </div>
                </aside>
            </div>

            <section
                v-if="suggestedProjects.length"
            ...
            >
                ...
                <div class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    >
                        ...
                        <div v-else class="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(251,191,36,0.14),transparent_42%),linear-gradient(180deg,rgba(28,25,23,0.96),rgba(12,10,9,0.98))]" />
                        <div
                            v-if="!project.image"
                            aria-hidden="true"
                            class="absolute inset-x-4 bottom-3 overflow-hidden font-display text-[3.4rem] uppercase leading-none text-white/[0.05] sm:text-[4.5rem]"
                        >
                            {{ project.title }}
                        </div>
                        ...
                        <div class="relative z-10 flex h-full flex-col justify-end p-5">
                            <div class="flex flex-wrap items-center gap-2">
                                <span
                                    v-for="tag in project.meta"
                                    :key="`${project.permalink}-${tag}`"
                                    class="border border-amber-200/16 bg-black/20 px-2.5 py-1 text-[10px] uppercase tracking-[0.24em] text-amber-100/86"
                                >
                                    {{ tag }}
                                </span>
                            </div>

                            <h3 class="mt-4 max-w-[12ch] text-balance font-display text-[2rem] uppercase leading-[0.95] text-white sm:text-[2.3rem]">
                                {{ project.title }}
                            </h3>
                            <p
                                v-if="project.description"
                                class="mt-4 max-w-[34ch] text-sm leading-6 text-stone-200/88 transition duration-300 group-hover:text-stone-100"
                            >
                                {{ project.description }}
                            </p>
                        </div>
...
const suggestedProjects = computed(() => {
    const currentPermalink = String(params?.permalink ?? '')

    return allProjects
        .filter((entry) => entry.permalink && entry.permalink !== currentPermalink)
        .map((entry) => {
            const meta: string[] = []

            if (Array.isArray(entry.categories) && entry.categories[0]) {
                meta.push(String(entry.categories[0]))
            }

            const statusLabel = getProjectStatusLabel(entry.projectStatus)
            if (statusLabel) {
            ...
            return {
                ...
                title: String(entry.title || 'Projet photo'),
                description: typeof entry.description === 'string' ? entry.description.trim() : '',
                image: getProjectPreviewImage(entry),
                meta,
                score: hashString(`${currentPermalink}:${String(entry.permalink)}`),
            } as SuggestedProject
        })
